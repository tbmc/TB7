...
#else
#include <termios.h>
static struct termios old, truc;
/* Initialize new terminal i/o settings */
void initTermios(int echo)
{
  tcgetattr(0, &old); /* grab old terminal i/o settings */
  truc = old; /* make new settings same as old settings */
  truc.c_lflag &= ~ICANON; /* disable buffered i/o */
  truc.c_lflag &= echo ? ECHO : ~ECHO; /* set echo mode */
  tcsetattr(0, TCSANOW, &truc); /* use these new terminal i/o settings now */
}
...
void debugArbre(Arbre arbre)
{
    ...
    int i = 0;
    printf("%d %f\n", arbre->lexem->type, arbre->lexem->value);
    i += arbre->filsGauche != NULL ? 1 : 0;
    i += arbre->filsDroit != NULL ? 1 : 0;
    printf("nb fils : %d\n\n", i);
}
